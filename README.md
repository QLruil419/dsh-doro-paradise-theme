...
| 版本 | 内容 | 推荐用途 |
| --- | --- | --- |
| `v1.3.1` | 当前版。默认上移侧栏立绘，并加入可调高度以避开插件按钮和分割线。 | 推荐 |
...
不要同时启用会重写背景、面板透明度或主题变量的全局皮肤，例如 `dsh-dream-skin`。它们可能互相覆盖，造成壁纸不显示、透明度异常或插件加载顺序问题。Doro Paradise `v1.3.1` 已内置常用的透明度、毛玻璃、液态玻璃和角色图层控制，通常不再需要额外皮肤插件。
