...
- 侧栏立绘高度可以在 48–320 px 间调节，避免新增插件按钮或分割线遮挡桃乐丝脚部与 Doro。
- 壁纸中央加入独立桃乐丝本体，右下角 Doro 可单独缩放，不再与背景图烘焙在一起。
- 壁纸采用融入伊甸园环境的安静站姿桃乐丝，侧栏则采用独立的玫瑰茶会坐姿立绘，两处不再复用同一形象。
- 所有设置会自动保存到 `$DSH_HOME/doro-paradise-theme.json`，即使 DSH 重启后更换端口也会恢复；浏览器 `localStorage` 仅作为快速预览与离线兜底。
- 设置页提供“保存设置”按钮，可立即写入磁盘并显示保存状态。
...
| 版本 | 内容 | 推荐用途 |
| --- | --- | --- |
| `v1.4.0` | 当前版。加入跨重启、跨端口的磁盘设置保存与手动保存按钮。 | 推荐 |
| `v1.3.1` | 默认上移侧栏立绘，并加入可调高度以避开插件按钮和分割线。 | 旧版 |
| `v1.3.0` | 壁纸与侧边栏改用两套不同的桃乐丝立绘，并优化壁纸融合感。 | 旧版 |
...
不要同时启用会重写背景、面板透明度或主题变量的全局皮肤，例如 `dsh-dream-skin`。它们可能互相覆盖，造成壁纸不显示、透明度异常或插件加载顺序问题。Doro Paradise `v1.4.0` 已内置常用的透明度、毛玻璃、液态玻璃和角色图层控制，通常不再需要额外皮肤插件。
